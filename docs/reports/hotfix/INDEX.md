...
- [Prompt autonome de sélection résiduelle HOTFIX-6A.10](_index/HOTFIX_NEXT_MERGE_PHASE_PROMPT.md).
...
- Merge global : Russie terminée statiquement; paquet Autriche/Croatie/Suisse clos par `HOTFIX_6A2F2`; DEI/VOC clos par `HOTFIX_6A3F_DEI_TARGETED_FIX_COMPLETE`; Balkan National Awakening clos par `HOTFIX_6A4F_BALKAN_NATIONAL_AWAKENING_1_13_COMPLETE`; `je_yugoslavia`, `je_risorgimento`, `je_greek_nationalism`, `je_great_eastern_crisis` et les huit entrées Sick Man clos par leurs rapports 6A.5F à 6A.9F. La prochaine phase est la sélection documentaire `HOTFIX_6A10_RESIDUAL_GLOBAL_SCRIPT_SELECTION`. L’activation Tanzimat différée 1776 reste un backlog de design distinct. L’ancien total annoncé de 26 deltas reste `UNVERIFIED`.
