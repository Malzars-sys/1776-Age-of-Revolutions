...
- Merge global : Russie terminée statiquement; paquet Autriche/Croatie/Suisse clos par `HOTFIX_6A2F2`; DEI/VOC clos par `HOTFIX_6A3F_DEI_TARGETED_FIX_COMPLETE`; Balkan National Awakening clos par `HOTFIX_6A4F_BALKAN_NATIONAL_AWAKENING_1_13_COMPLETE`; `je_yugoslavia`, `je_risorgimento`, `je_greek_nationalism`, `je_great_eastern_crisis`, les huit entrées Sick Man, les deux entrées roumaines, les deux pinning et les deux régions stratégiques du colonialisme portugais sont clos. Merchant Banking GEN/VEN est également clos statiquement et en runtime par `HOTFIX_6A13F_GEN_VEN_MERCHANT_BANKING_STARTING_LAW_COMPLETE`. L'audit Navigation Acts 6A.14R est clos sans gameplay. Le runtime HBC de 6A.14H confirme une fusion stable et sans clé brute de `hubson` et `hudson`; l'intention canonique reste une décision de design non résolue et ni 6A.14F ni 6A.14HF ne sont sélectionnées. GBR croise NAVY, BIC et la chaîne américaine restent protégées. Coup reste un audit séparé non sélectionné ; Imperialism, Tanzimat et la répétition française du nom portugais ne sont pas autorisés. L'équilibrage 1776 de Merchant Banking est un backlog post-hotfix non bloquant. Aucune phase suivante n'est sélectionnée et l'ancien total annoncé de 26 deltas reste `UNVERIFIED`.
