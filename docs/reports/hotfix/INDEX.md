...
- [Prompt autonome d’alignement des régions stratégiques portugaises HOTFIX-6A.12F](_index/HOTFIX_NEXT_MERGE_PHASE_PROMPT.md).
...
- Merge global : Russie terminée statiquement; paquet Autriche/Croatie/Suisse clos par `HOTFIX_6A2F2`; DEI/VOC clos par `HOTFIX_6A3F_DEI_TARGETED_FIX_COMPLETE`; Balkan National Awakening clos par `HOTFIX_6A4F_BALKAN_NATIONAL_AWAKENING_1_13_COMPLETE`; `je_yugoslavia`, `je_risorgimento`, `je_greek_nationalism`, `je_great_eastern_crisis`, les huit entrées Sick Man, les deux entrées roumaines et les deux pinning du colonialisme portugais sont clos. 6A.12 sélectionne uniquement l’alignement des deux régions stratégiques portugaises pour `HOTFIX_6A12F_PORTUGUESE_COLONIALISM_STRATEGIC_REGION_KEYS_1_13_ALIGNMENT`, à exécuter après commit manuel de cette phase documentaire. Merchant Banking, Navigation Acts, Coup, Imperialism, Tanzimat et la répétition française du nom portugais restent séparés. L'ancien total annoncé de 26 deltas reste `UNVERIFIED`.
