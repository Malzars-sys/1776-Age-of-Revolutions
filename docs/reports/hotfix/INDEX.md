...
- Merge global : Russie terminée statiquement; paquet Autriche/Croatie/Suisse clos par `HOTFIX_6A2F2`; DEI/VOC clos par `HOTFIX_6A3F_DEI_TARGETED_FIX_COMPLETE`; Balkan National Awakening clos par `HOTFIX_6A4F_BALKAN_NATIONAL_AWAKENING_1_13_COMPLETE`; `je_yugoslavia`, `je_risorgimento`, `je_greek_nationalism`, `je_great_eastern_crisis`, les huit entrées Sick Man, les deux entrées roumaines, les deux pinning et les deux régions stratégiques du colonialisme portugais sont clos. 6A.13 sélectionne exclusivement `HOTFIX_6A13F_GEN_VEN_MERCHANT_BANKING_STARTING_LAW`, à exécuter après commit manuel de la phase documentaire. Navigation Acts et Coup restent des audits séparés ; Imperialism, Tanzimat et la répétition française du nom portugais ne sont pas autorisés. L'ancien total annoncé de 26 deltas reste `UNVERIFIED`.
