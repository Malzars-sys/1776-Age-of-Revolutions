...
- [Prompt autonome de sélection résiduelle HOTFIX-6A.7](_index/HOTFIX_NEXT_MERGE_PHASE_PROMPT.md).
...
- Merge global : Russie terminée statiquement; paquet Autriche/Croatie/Suisse clos par `HOTFIX_6A2F2`; DEI/VOC clos par `HOTFIX_6A3F_DEI_TARGETED_FIX_COMPLETE`; Balkan National Awakening clos par `HOTFIX_6A4F_BALKAN_NATIONAL_AWAKENING_1_13_COMPLETE`; `je_yugoslavia` clos par `HOTFIX_6A5F_YUGOSLAVIA_JE_PINNING_1_13_COMPLETE`; `je_risorgimento` clos par `HOTFIX_6A6F_ITALIAN_UNIFICATION_JE_PINNING_1_13_COMPLETE`. La prochaine phase est `HOTFIX_6A7_RESIDUAL_GLOBAL_SCRIPT_SELECTION`, strictement documentaire. L’ancien total annoncé de 26 deltas reste `UNVERIFIED`.
